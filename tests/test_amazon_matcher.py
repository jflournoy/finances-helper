...
class TestCLI:
    ...
    def test_it_cli_full_pipeline_with_sandbox_mode_set(self, tmp_path, monkeypatch, capsys):
        """Pipeline completes successfully even when YNAB_SANDBOX_MODE=1 is set.

        After lazy-init refactor (#155), the pipeline (which never writes) is unaffected
        by sandbox env state. This test verifies that behavior.
        """
        import sys
        from zipfile import ZipFile
        ...
        monkeypatch.setenv("YNAB_SANDBOX_MODE", "1")  # explicitly set — pipeline must still work
    ...
    def test_it_cli_full_pipeline(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
        monkeypatch.setenv("YNAB_DEFAULT_BUDGET", "budget-uuid-test")
        # YNAB_SANDBOX_MODE is intentionally NOT cleared here. The pipeline only reads
        # transactions and writes a changeset file — it never calls create_transactions().
        # Sandbox init is lazy (deferred to first write), so sandbox env state is irrelevant
        # for this test. If a future change adds a write path, this test will need a mock
        # for create_transactions() to avoid hitting real YNAB.
