...
# Import the script (will fail if import-time errors, which is good)
import sys
...
def test_writes_markdown_and_json_to_out_dir(temp_out_dir, monkeypatch):
    ...
    with patch("sys.argv", ["spend_advice.py", "--days", "90", "--out-dir", temp_out_dir]):
        with patch("spend_advice.YNABClient") as mock_client_cls:
            ...
            # Return empty txns and simple categories
    ...
    md_files = list(Path(temp_out_dir).glob("spend-advice-*.md"))
    json_files = list(Path(temp_out_dir).glob("spend-advice-*.json"))
...
def test_markdown_contains_insights_header(temp_out_dir, monkeypatch):
    ...
    # Should have at least some markdown structure
